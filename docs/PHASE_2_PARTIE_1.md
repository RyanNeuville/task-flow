...
### Table `users`
*Objectif : Gérer l'authentification et isoler les données de chaque personne.*
...
### Table `projects`
*Objectif : Regrouper les tâches par contexte ou dossier.*
* `id` : INT (Auto-increment, Primary Key)
* `title` : VARCHAR(100)
...
### Table `tasks`
*Objectif : Le coeur de l'app de productivité.*
* `id` : INT (Auto-increment, Primary Key)
* `title` : VARCHAR(255)
...
* `project_id` : INT (Foreign Key -> `projects.id` ON DELETE CASCADE)
* `created_at` : DATETIME
* `updated_at` : DATETIME (Mis à jour automatiquement via trigger ou code)
* `deleted_at` : DATETIME NULL (Soft Delete)

---
*Note de conception : Le "ON DELETE CASCADE" permet de supprimer automatiquement toutes les tâches si le projet est supprimé, et tous les projets si l'utilisateur supprime son compte. C'est propre et évite les données orphelines.*
